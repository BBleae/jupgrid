...
function envload(): [Keypair, string] {
  ...
  while (true) {
    if (process.env.FLAG) {
      ...
      const encryptedKey = cryptr.encrypt(process.env.PRIVATE_KEY)
      const encryptedFlag = cryptr.encrypt(encflag)
      fs.writeFileSync(
        envFilePath,
...
function loaduserSettings() {
  try {
    const data = fs.readFileSync('userSettings.json', 'utf-8')
    const userSettings = JSON.parse(data)
    return userSettings
  }
  catch (error) {
    console.log('No user data found. Starting with fresh inputs.', error)
    initialize()
  }
}
