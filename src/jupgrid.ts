...
const configVersion = currentVersion
...
let stopLossUSD = null
let infinityTarget = null
let loaded = false
let openOrders = []
let checkArray = []
let tokens = []
...
let startPrice = null
let spread = null
const spreadbps = null
const initBalanceA = 0
const initUsdBalanceA = 0
const initBalanceB = 0
const initUsdBalanceB = 0
const currBalanceA = 0
const currBalanceB = 0
const currUSDBalanceA = 0
const currUSDBalanceB = 0
const initUsdTotalBalance = 0
const currUsdTotalBalance = 0
let tokenRebalanceValue: number | null = null
let tokenARebalanceValue = 0
let tokenBRebalanceValue = 0
...
// stopLoss = false,
let maxJitoTip = 0
const infinityBuyInputLamports = null
const infinityBuyOutputLamports = null
const infinitySellInputLamports = null
const infinitySellOutputLamports = null
const counter = 0
const askForRebalance = true
const rebalanceCounter = 0
const newPriceBUp = null
const newPriceBDown = null
let lastKnownPrice: number | null = null
const currentTracker = null
...
function formatElapsedTime(startTime: Date) {
  ...
  const elapsedTime = currentTime.getTime() - startTime.getTime() // Difference in milliseconds
...
async function updateUSDVal(mintAddress, balance, decimals) {
  try {
    const price = await fetchPrice(mintAddress)
    const balanceLamports = Math.floor(balance * 10 ** decimals)
    const usdBalance = balanceLamports * price
    const usdBalanceLamports = usdBalance / 10 ** decimals
...
async function updatePrice() {
  ...
  while (retries < maxRetries) {
    try {
      const newPrice = await fetchPrice(selectedAddressB)
      ...
      console.error(`Fetch price failed. Attempt ${retries + 1} of ${maxRetries}`, error)
    }
    retries++
  }
  ...
  async function getSOLBalanceAndUSDC() {
    ...
    if (selectedTokenA === 'SOL' || selectedTokenB === 'SOL') {
      try {
        ...
        usdBalance = response.data.outAmount / 10 ** 6 || 0
        tokenRebalanceValue = response.data.outAmount / (lamports / 10 ** 3)
      }
      catch (error) {
    ...
    if (tokenAccounts.value.length > 0) {
      ...
      if (mintAddress !== USDC_MINT_ADDRESS) {
        const queryParams = {
          ...
          outputMint: USDC_MINT_ADDRESS,
          amount: Math.floor(balance * 10 ** decimals),
          slippageBps: 0,
        }
        ...
        try {
          ...
          usdBalance = response.data.outAmount / 10 ** 6
          tokenRebalanceValue = response.data.outAmount / (balance * 10 ** 6)
        }
        catch (error) {
...
Token Decimals: ${token.decimals}`)
  ...
  while (!validInfinityTarget) {
    ...
    infinityTarget = Math.floor(Number.parseFloat(infinityTargetInput))
    if (
  ...
  if (userSettings.spread) {
    validSpread = !isNaN(Number.parseFloat(userSettings.spread))
    if (!validSpread) {
      console.log(
...
Recommend >0.3% to cover Jupiter Fees, but 1% or greater for best performance:`,
    ...
    spread = Number.parseFloat(spreadInput)
    if (!isNaN(spread)) {
  ...
  if (userSettings.stopLossUSD) {
    validStopLossUSD = !isNaN(Number.parseFloat(userSettings.stopLossUSD))
    if (!validStopLossUSD) {
      console.log(
...
(Enter 0 for no stoploss) `,
    ...
    stopLossUSD = Number.parseFloat(stopLossUSDInput)
...
(Default 0.0002 SOL, Minimum 0.00001): `,
    ...
    if (maxJitoTipQuestion.trim() === '') {
      ...
      const parsedMaxJitoTip = Number.parseFloat(maxJitoTipQuestion.trim())
...
(minimum 100ms, recommended/default > 5000ms): `,
    ...
    if (monitorDelayQuestion.trim() === '') {
      ...
      const parsedMonitorDelay = Number.parseInt(monitorDelayQuestion.trim())
...
async function infinityGrid() {
  ...
  const balanceALamports = balanceA * 10 ** selectedDecimalsA
  const balanceBLamports = balanceB * 10 ** selectedDecimalsB
  ...
  const currentValueUSD = balanceBLamports / 10 ** selectedDecimalsB * averageMarketPrice

  // Calculate the target value of TokenB in USD at the new prices
  const targetValueUSDUp = balanceBLamports / 10 ** selectedDecimalsB * newPriceBUp
  const targetValueUSDDown = balanceBLamports / 10 ** selectedDecimalsB * newPriceBDown

  // Calculate the initial lamports to sell and buy
  const lamportsToSellInitial = Math.floor((targetValueUSDUp - infinityTarget) / newPriceBUp * 10 ** selectedDecimalsB / 0.998)
  const lamportsToBuyInitial = Math.floor((infinityTarget - targetValueUSDDown) / newPriceBDown * 10 ** selectedDecimalsB / 0.998)

  // Adjust the lamports to buy based on the potential cancellation of the sell order
  const lamportsToBuy = lamportsToBuyInitial - lamportsToSellInitial

  // lamportsToSell remains the same as lamportsToSellInitial
  const lamportsToSell = lamportsToSellInitial

  // Calculate the expected USDC for the sell and buy
  const decimalDiff = selectedDecimalsB - selectedDecimalsA
  const expectedUSDCForSell = (lamportsToSell * newPriceBUp) / 10 ** selectedDecimalsB
  const expectedUSDCForBuy = (lamportsToBuy * newPriceBDown) / 10 ** selectedDecimalsB
  const expectedUSDCForSellLamports = Math.floor((lamportsToSell * newPriceBUp) / 10 ** decimalDiff)
  const expectedUSDCForBuyLamports = Math.floor((lamportsToBuy * newPriceBDown) / 10 ** decimalDiff)
...
async function cancelOrder(target = [], payer) {
  ...
  for (let i = 0; i < retryCount; i++) {
    ...
    try {
      ...
      await delay(2000)
      if (i === retryCount - 1)
        throw error // If last retry, throw error
...
async function balanceCheck() {
  ...
  const currBalanceALamports = currBalanceA * 10 ** selectedDecimalsA
  const currBalanceBLamports = currBalanceB * 10 ** selectedDecimalsB
...
Attempting to swap ${chalk.cyan(adjustmentA / 10 ** selectedDecimalsA)} ${chalk.cyan(selectedTokenA)} to ${chalk.magenta(selectedTokenB)}`,
      )
      process.exit(0)
    }
    console.log(
      `Need to trade ${chalk.cyan(adjustmentA / 10 ** selectedDecimalsA)} ${chalk.cyan(selectedTokenA)} to ${chalk.magenta(selectedTokenB)} to balance.`,
    )
...
Attempting to swap ${chalk.magenta(adjustmentB / 10 ** selectedDecimalsB)} ${chalk.magenta(selectedTokenB)} to ${chalk.cyan(selectedTokenA)}`,
      )
      process.exit(0)
    }
    console.log(
      `Need to trade ${chalk.magenta(adjustmentB / 10 ** selectedDecimalsB)} ${chalk.magenta(selectedTokenB)} to ${chalk.cyan(selectedTokenA)} to balance.`,
    )
    const userConfirmation = await confirmTransaction()
    if (userConfirmation) {
      const rebalanceTx = await rebalanceTokens(
...
async function updateMainDisplay() {
  ...
  const { multiplier, quantity } = await formatTokenPrice(newPrice)
  const adjustedNewPrice = newPrice * multiplier
  const adjustedNewPriceBUp = newPriceBUp * multiplier
  const adjustedNewPriceBDown = newPriceBDown * multiplier
  if (iteration === 0) {
    currentTracker = Array.from({ length: 50 }).fill(adjustedNewPrice)
    sellPrice = Array.from({ length: 50 }).fill(adjustedNewPriceBUp)
    buyPrice = Array.from({ length: 50 }).fill(adjustedNewPriceBDown)
  }
...
-
Sell Order Price: ${newPriceBUp.toFixed(9)} - Selling ${chalk.magenta(Math.abs(infinitySellInputLamports / 10 ** selectedDecimalsB))} ${chalk.magenta(selectedTokenB)} for ${chalk.cyan(Math.abs(infinitySellOutputLamports / 10 ** selectedDecimalsA))} ${chalk.cyan(selectedTokenA)}
Current Price ${quantity}:`)
  console.log(asciichart.plot([currentTracker, buyPrice, sellPrice], config))
  console.log(`Buy Order Price: ${newPriceBDown.toFixed(9)} - Buying ${chalk.magenta(Math.abs(infinityBuyOutputLamports / 10 ** selectedDecimalsB))} ${chalk.magenta(selectedTokenB)} for ${chalk.cyan(Math.abs(infinityBuyInputLamports / 10 ** selectedDecimalsA))} ${chalk.cyan(selectedTokenA)}\n`)
}
