...
async function jitoTipCheck() {
  ...
  // Handle messages
  tipws.on('message', (data: AllowSharedBufferSource) => {
...
async function jitoController(task) {
  ...
  switch (task) {
    case 'cancel':
      result = await jitoCancelOrder(task) || 'cancel with no result'
      break
    case 'infinity':
      result = await jitoSetInfinity(task) || 'infinity with no result'
      break
    case 'rebalance':
      result = await jitoRebalance(task) || 'rebalance with no result'
      break
    default:
      // unintended code
...
async function handleJitoBundle(task, ...transactions) {
  ...
  try {
    ...
    const instructionsSub: TransactionInstruction[] = []
    ...
    console.log(
      `Jito Fee: ${limitedTipValueInLamports / 10 ** 9} SOL`,
    )
  ...
  for (let i = 0; i <= maxRetries; i++) {
    try {
      ...
      if (response.status === 429) {
        const waitTime = Math.min(MIN_WAIT * 2 ** i, MAX_WAIT)
